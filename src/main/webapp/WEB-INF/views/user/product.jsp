...
<body>
	<div class="container" style="margin-top: 4%;">
					...
					<div class="card product-card card-h-set-320">
						...
						<p class="card-text text-center mb-4" >
							...
							<c:if test="${product.gia ne product.giamoi}">
								<del class="mr-3">
									<strong class ="gia-tien">${product.gia}</strong>
								</del>
							</c:if>
							<!-- Giá sau khi giảm (màu đỏ) -->
							<span style="color: red;"><strong class ="gia-tien">${product.giamoi}</strong></span>
						</p>
					</div>
				</div>
		...
		<div class="row" style="padding-bottom: 100%;">
			<div class="col-md-6">
				<div class="display-1">Helo looooo</div>
...
<script type="text/javascript">
    function dinhDangTienVND() {
        // Lấy tất cả các phần tử có lớp "gia-tien"
        var giaTienElements = document.querySelectorAll('.gia-tien');
        
        // Tạo đối tượng Intl.NumberFormat để định dạng tiền tệ Việt Nam
        ...
        giaTienElements.forEach(function(element) {
            // Lấy giá trị hiện tại của phần tử
            var gia = parseFloat(element.textContent.trim());
            
            // Kiểm tra nếu giá trị là số hợp lệ
            if (!isNaN(gia)) {
                // Định dạng giá trị thành tiền tệ Việt Nam
                var formattedPrice = formatter.format(gia);
                
                // Cập nhật lại nội dung của phần tử với giá trị đã định dạng
                element.textContent = formattedPrice;
            }
        });
    }
    
    // Gọi hàm để định dạng khi trang được tải
    window.onload = dinhDangTienVND;
</script>
