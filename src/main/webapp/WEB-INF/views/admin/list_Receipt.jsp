								...
								<div class="table-wrap">
									...
									<thead>
										...
										<c:forEach var="pn" items="${receipts}">
											<tr class="table-row">
												<td class="table-data">${pn.mapn}</td>
												<td class="table-data">${pn.ngaynhap}</td>
												<td class="table-data">${pn.nhacungcap.tenncc}</td>
												<td><a type="button"
													class="btn btn  btn-success float-right"
													style="color: white" data-toggle="modal"
													data-target="#modal-${pn.mapn}"> Chi tiết </a>

													<div class="modal fade" id="modal-${pn.mapn}">
																						...
																						<div class="table-wrap">
																							<table class="table table-striped">
																								<tr class="table-row">
																									...
																									<th class="table-heading">Tên size</th>
																									<th class="table-heading">Số lượng</th>
																									<th class="table-heading">Giá nhập</th>

																								</tr>
																								 <c:forEach var="sp" items="${receiptDao.getListProduct_From_Reeipt(pn.mapn)}">
																									<tr class="table-row">
																										 <td class="table-data">${sp.ctsize.mathang.getTenmh()}</td>
																										 <td class="table-data">${sp.ctsize.size.getTensize()}</td>
																										<td class="table-data">${sp.getSoluong()}</td>
																										<td class="table-data">${sp.getDongia()}</td> 
																									</tr>
																								</c:forEach>  
																							</table>
