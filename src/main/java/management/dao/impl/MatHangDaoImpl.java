...
public class MatHangDaoImpl implements IMatHangDao {
	...
	@Override
	public Integer updateProduct(Mathang pd) {
	...
	@Override
	public List<Mathang> getAllSP() {
		Session session = sessionFactory.openSession();
		//String hql = "FROM Product p WHERE p.mamh NOT IN (SELECT d.product.id FROM DetailsUpdatePrice d)";
	...
	@Override
	public List<Loaimh> getAllLoai() {
		Session session = sessionFactory.openSession();
		String hql = "FROM Loaimh";
		Query query = session.createQuery(hql);
		List<Loaimh> list = query.list();
		return list;
	}

	@Override
	public Mathang getSP(Integer masp) {
		Session session = sessionFactory.openSession();
		try {
			String hql = "FROM Mathang WHERE  mamh = :masp";
			Query query = session.createQuery(hql);
			query.setParameter("masp", masp);
			Mathang product = (Mathang) query.uniqueResult();
			return product;
	...
	public List<Mathang> getProductOfCategory(String category) {
		Session session = sessionFactory.openSession();
		List<Mathang> list = null;
		String hql = "FROM Mathang M WHERE M.loaimh.maloaimh = :loaimhId";
		Query query = session.createQuery(hql);
	...
	public List<Mathang> getMathangAndTotalSoluongTop(int top) {
		...
		String hql = "SELECT M.* " +
	             "FROM mathang M " +
	             "INNER JOIN (" +
	             "    SELECT C.MAMH, SUM(C.soluong) AS TongSoLuong " +
	             "    FROM Ctpd C " +
	             "    GROUP BY C.MAMH " +
	             ") AS SubQuery ON M.MAMH = SubQuery.MAMH " +
	             "ORDER BY SubQuery.TongSoLuong DESC " +
	             "LIMIT " + top;
