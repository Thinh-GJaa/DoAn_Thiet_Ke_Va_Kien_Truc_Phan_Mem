...
public class NhapHangDaoImpl implements INhapHangDao {
	...
	@Override
	public List<Phieunhap> getListReceipt() {

		Session session = sessionFactory.openSession();

		String hgl = "From Phieunhap";
	...
	@Override 
	public List<Ctpn> getListProduct_From_Reeipt(Integer id )
	{
		Session session = sessionFactory.openSession();
		String hql = " FROM Ctpn ctpn  WHERE ctpn.phieunhap.mapn = :id";
		Query query = session.createQuery(hql);
		query.setParameter("id", id);
		List <Ctpn> list = query.list();
		return list;
	}
	/*
	 * @Override public List<Ctsize> getListProduct_From_Reeipt(Integer id ) {
	 * 
	 * Session session = sessionFactory.openSession();
	 * 
	 * String hql =
	 * "SELECT cts FROM Ctpn ctpn JOIN ctpn.ctsize cts WHERE ctpn.phieunhap.mapn = :id"
	 * ; Query query = session.createQuery(hql); query.setParameter("id", id);
	 * 
	 * List <Ctsize> list = query.list();
	 * 
	 * 
	 * Set<Ctsize> l = new HashSet<>(list);
	 * 
	 * 
	 ...
	 * 
	 * 
	 * HashMap<Ctsize, Long> map = new HashMap<>();
	 * 
	 * 
	 * for (Ctsize product : list) { String hqlCount =
	 * "Select count(*) from Seri s where s.product.id = :pdid and s.receipt.id = :reid"
	 * ; query = session.createQuery(hqlCount); query.setParameter("pdid",
	 * product.getId()); query.setParameter("reid", id); map.put(product, (Long)
	 * query.list().get(0)); }
	 * 
	 * 
	 * 
